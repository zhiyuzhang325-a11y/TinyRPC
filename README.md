...
早期客户端使用 `std::async(launch::async)` 为每次 RPC 调用创建独立线程，通过 `std::future` 返回结果。压测发现 QPS 仅约 2000，单次延迟高达 4500μs。
...
|------|----------------------|---------------------|------|
| QPS | ~2,000 | ~9,300 | **+365%** |
| p50 延迟 | ~4,500μs | ~1,000μs | **-78%** |

### 并发度测试（SubReactor=4，worker=4）

| 客户端线程数 | QPS | p50 (μs) | p99 (μs) | p999 (μs) |
|-------------|------|----------|----------|-----------|
| 5 | 8,509 | 555 | 925 | 1,198 |
| 10 | 9,326 | 997 | 2,104 | 2,581 |
| 20 | 9,432 | 1,952 | 4,754 | 6,177 |
| 50 | 9,806 | 4,582 | 13,902 | 19,953 |

吞吐量在 10 线程时基本饱和（~9,300 QPS），继续增加并发线程 QPS 提升有限但尾部延迟急剧上升（50 线程 p99 达 14ms）。

### 服务端配置对比

- **SubReactor 数量**（2 / 4 / 8 / 16，固定 worker=4）：QPS 均在 9,200–9,500 范围，无显著差异
- **Worker 线程数**（1 / 2 / 4 / 8 / 16 per SubReactor，固定 SubReactor=4）：QPS 均在 9,200–9,500 范围，无显著差异
...
|---------|------|----------|----------|
| 单实例 | 9,326 | 997 | 2,104 |
| 3 实例同机 | 8,231 | 1,139 | 2,425 |

同机多实例因资源争用 QPS 略有下降，但 ZooKeeper 服务发现与 Round-Robin 负载均衡工作正常，成功率 100%。多实例部署的价值在于跨机器水平扩展。
